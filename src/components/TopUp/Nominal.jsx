            ...
            <div>
              <div className="mb-6">
                <div className="flex items-center mb-2">
                  ...
                  <div className="relative w-full h-1 bg-gray-300 rounded">
                    <div className="h-1 bg-orange-500 rounded" style={{width: '66.66%'}} />
                  </div>
                </div>
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                <button 
                  ...
                  className={`px-6 py-2 text-sm font-semibold rounded-lg shadow-md focus:outline-none ${activeView === 'PASS' ? 'text-white bg-orange-500' : 'text-gray-600 bg-gray-200 hover:bg-gray-300'}`}
                >
                  Twilight Pass
                </button>
              ...
              {activeView === 'DIAMOND' && (
                  <div className="animate-fade-in grid grid-cols-2 md:grid-cols-3 gap-4">
                    {diamondPackages.length > 0 ? diamondPackages.map((pkg) => (
                      <div key={pkg.id_packages} onClick={() => handleSelect(pkg)} className={`relative p-4 border rounded-lg cursor-pointer transition-all duration-200 h-28 flex flex-col items-center justify-center ${selectedItem?.id_packages === pkg.id_packages ? "border-orange-500 bg-orange-50 shadow-lg" : "border-gray-300 bg-white hover:border-orange-400"}`}>
                        <p className="font-bold text-center text-gray-800">{pkg.name}</p>
                        <p className="text-sm text-center text-gray-500 mt-1">Rp {pkg.price.toLocaleString("id-ID")}</p>
                      </div>
                    )) : <p className="col-span-full text-center text-gray-500">Tidak ada paket diamond tersedia.</p>}
                  </div>
              )}
              
              {activeView === 'PASS' && (
                  <div className="animate-fade-in grid grid-cols-2 md:grid-cols-3 gap-4">
                    {passPackages.length > 0 ? passPackages.map((pkg) => (
                      <div key={pkg.id_packages} onClick={() => handleSelect(pkg)} className={`relative p-4 border rounded-lg cursor-pointer transition-all duration-200 h-28 flex flex-col items-center justify-center ${selectedItem?.id_packages === pkg.id_packages ? "border-orange-500 bg-orange-50 shadow-lg" : "border-gray-300 bg-white hover:border-orange-400"}`}>
                        <p className="font-bold text-center text-gray-800">{pkg.name}</p>
                        <p className="text-sm text-center text-gray-500 mt-1">Rp {pkg.price.toLocaleString("id-ID")}</p>
                      </div>
                    )) : <p className="col-span-full text-center text-gray-500">Tidak ada paket pass tersedia.</p>}
                  </div>
              )}
            </div>
                ...
                <>
                  ...
                  <div className="mt-6">
                    <div className="flex flex-col gap-4">
                      <button onClick={nextStep} className="w-full py-3 px-6 bg-orange-500 text-white font-bold rounded-lg ...">Lanjut</button>
                      <button onClick={prevStep} className="w-full py-3 px-6 bg-white text-orange-500 font-bold rounded-lg border ...">Kembali</button>
                    </div>
                  </div>
                </>
              ) : (
